...
class ModelUserOrg extends Model
{
	...
	public function getHashOrgs()
	{
		$sql = "SELECT O.*, COUNT(U.user_id) AS total FROM {$this->_opt} O LEFT JOIN cc_user U ON U.org_id = O.org_id GROUP BY O.org_id";

		return $this->hash_sql($sql, 'org_id');
	}
