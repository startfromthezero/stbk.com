...
class ControllerToolLinks extends Controller
{
	...
	{
		return $this->view('template/tool/links_list.tpl', $vrs);
	}
	...
	public function oper(){
		...
		{
			$this->registry->model('tool/links');
			$vrs             = $this->model_tool_links->get($links_id);
			$site            = explode(',', $vrs['show_site']);
			$vrs['homePage'] = $site[0];
			$vrs['subPage']  = $site[1];
		}
		return $this->view('template/tool/links_add.tpl', $vrs);
	}
	...
	public function addLinks(){
		...
		if(isset($data['links_id'])&& !empty($data['links_id'])){
			$this->model_tool_links->update($data);
		}else{
			$this->model_tool_links->insert($data);
		}
	...
	public function getList()
	{
		...
		$links = $this->model_tool_links->getLinksList($data);
		...
		//$total = $this->model_tool_links->getLinksList($data, true);
