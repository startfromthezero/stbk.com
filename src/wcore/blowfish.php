 ...
 * 版本: $Id: blowfish.php 339 2016-07-01 02:45:07Z zhangsl $
 *
...
class wcore_blowfish
{
	 ...
	 *
	 * @access public
	 * @param  int    $mode the block mode to use
	 * @param  string $key  the ley used to crypt
	 */
	public function __construct($mode = MODE_CBC, $key = null)
	{
	...
	private function _cbc_encrypt($plain)
	{
		...
		for ($i = 0; $i < $plainsize; $i = $i + $blocksize)
		{
			...
			if (strlen($block) < $blocksize)
			{
				$block = str_pad($block, $blocksize, "\0", STR_PAD_LEFT);// pad block with '\0'
			}
