...
class Tax extends modules_mem
{
	...
	{
		$tax_row = $this->sdb()->fetch_row("SELECT name FROM " . DB_PREFIX . "tax_rate WHERE tax_rate_id = '{$tax_rate_id}'");
		if (!empty($tax_row))
		{
			return $tax_row['name'];
		}
	...
	public function getRates($value, $tax_class_id)
	{
		...
		{
			$tax_rows = $this->sdb()->fetch_all("SELECT tr2.tax_rate_id, tr2.name, tr2.rate, tr2.type, tr1.priority
			FROM " . DB_PREFIX . "tax_rule tr1 LEFT JOIN " . DB_PREFIX . "tax_rate tr2 ON (tr1.tax_rate_id = tr2.tax_rate_id)
			INNER JOIN " . DB_PREFIX . "tax_rate_group tr2cg ON (tr2.tax_rate_id = tr2cg.tax_rate_id)
			...
			WHERE tr1.tax_class_id = '{$tax_class_id}' AND tr1.based = 'shipping' AND tr2cg.customer_group_id = '{$customer_group_id}'
			AND z2gz.country_id = '{$this->shipping_address['country_id']}' AND (z2gz.zone_id = '0' OR z2gz.zone_id = '{$this->shipping_address['zone_id']}')
			ORDER BY tr1.priority ASC");
			foreach ($tax_rows as $result)
			{
		...
		if ($this->payment_address)
		{
			...
			LEFT JOIN " . DB_PREFIX . "zone_to_geo_zone z2gz ON (tr2.geo_zone_id = z2gz.geo_zone_id)
			LEFT JOIN " . DB_PREFIX . "geo_zone gz ON (tr2.geo_zone_id = gz.geo_zone_id)
			WHERE tr1.tax_class_id = '{$tax_class_id}' AND tr1.based = 'payment' AND tr2cg.customer_group_id = '{$customer_group_id}'
			AND z2gz.country_id = '{$this->payment_address['country_id']}' AND (z2gz.zone_id = '0' OR z2gz.zone_id = '{$this->payment_address['zone_id']}')
			ORDER BY tr1.priority ASC");
			foreach ($tax_rows as $result)
			{
				$tax_rates[$result['tax_rate_id']] = array(
		...
		if ($this->store_address)
		{
			...
			LEFT JOIN " . DB_PREFIX . "zone_to_geo_zone z2gz ON (tr2.geo_zone_id = z2gz.geo_zone_id)
			LEFT JOIN " . DB_PREFIX . "geo_zone gz ON (tr2.geo_zone_id = gz.geo_zone_id)
			WHERE tr1.tax_class_id = '{$tax_class_id}' AND tr1.based = 'store' AND tr2cg.customer_group_id = '{$customer_group_id}'
			AND z2gz.country_id = '{$this->store_address['country_id']}' AND (z2gz.zone_id = '0' OR z2gz.zone_id = '{$this->store_address['zone_id']}')
			ORDER BY tr1.priority ASC");
			foreach ($tax_rows as $result)
			{
				$tax_rates[$result['tax_rate_id']] = array(
