 ...
 * 慧佳工作室 -> hoojar studio
 *
 * 模块: $Id: setting.php 356 2016-07-05 01:43:21Z zhangsl $
 * 简述: 网站各大参数设置 (注此文件需COPY一份到此目录下并改名为setting.php)
 * 作者: woods·zhang  ->  hoojar@163.com -> http://www.cyagps.com/
 *
...
ini_set('session.name',		'gprsid');						//设置SESSION名称
...
define('USE_CDN_JS2CSS',	1);			//是否使用CDN静态文件(JS,CSS)
...
define('SITE_MD5_KEY',		'*#20spt15');//网站MD5密匙
 ...
 * HTTP URL
 */
define('DOMAIN_NAME',	isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'gprs.yunovo.cn');//当前域名
 ...
 * 微信支付相关参数
 */
define('WX_APPID',		'wxff00d74721c04ed0');				//公共平台APPID
define('WX_APPSECRET',	'ed6a5919505b0016d27bc24c5cc85ff9');//公共平台appsecret

/**
 * 联通接口相关参数
 */
define('UNICOM_KEY', '9c5987f81c84850b1165834d041bd33a');	//接口密钥
define('UNICOM_URL', 'http://122.143.21.162/api.php');		//接口地址

/**
 * 数据库读取类型
 */
define('DB_PREFIX',		'cc_');					//数据库表前缀
...
define('RDS_SERVERS', json_encode($rds_server));
unset($rds_server);
 ...
 */
$cdn_urls	= array();
$cdn_urls[]	= HTTP_STORE;//图片主机域名1
define('CDN_URLS', json_encode($cdn_urls));
unset($cdn_urls);
 ...
 */
$smtp_server		= array();
$smtp_server['host']= 'mail.cyagps.com';	//SMTP服务器主机
$smtp_server['port']= 25;					//SMTP服务器端口
$smtp_server['user']= 'getpwd@cyagps.com';	//SMTP登录用户账号
